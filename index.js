...
async function getAllfoldersRecursively(drive, id, parents, folders) {
    ...
    const temp = files.map((e) => {
        return {name: e.name, id: e.id, parent: e.parents[0], tree: parents.concat(id)};
    });
    if (temp.length > 0) {
...
function init(e, callback) {
    ...
    if (!chkAuth && rootId) {
        callback("All folders in Google Drive cannot be retrieved using API key. Please use OAuth2.", null);
        return;
    }
